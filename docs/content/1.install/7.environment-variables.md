...
To disable password login, set `SPARKY_FITNESS_DISABLE_EMAIL_LOGIN=true` and unset `SPARKY_FITNESS_FORCE_EMAIL_LOGIN` or set it to `false`. The example environment file enables `SPARKY_FITNESS_FORCE_EMAIL_LOGIN` by default.
...
- **`SPARKY_FITNESS_OIDC_PROVIDER_SLUG`**: URL-safe provider id (e.g. `authentik`). Required for env provider upsert. To update an existing provider, the slug must match its stored provider ID exactly (e.g. `oidc-authentik` must remain `oidc-authentik`, not `authentik`).
