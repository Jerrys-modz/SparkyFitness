...
- The watchOS companion app lives entirely under `targets/watch/` (a `@bacons/apple-targets` `type: 'watch'` extension, `expo-target.config.js`), iOS-only — there is no Wear OS/Android equivalent. Layers: `Presentation/` (SwiftUI views, `ContentView`'s `TabView` — swipe order is body order, not the `Page` enum's), `Application/` (`CheckInStore`, `WorkoutSessionStore` — `@MainActor ObservableObject` singletons, injected in `SparkyFitnessWatchApp.swift`), `Infrastructure/` (`WatchSessionManager` wrapping `WCSession`; `WorkoutHealthKitController` wrapping `HKWorkoutSession`/`HKLiveWorkoutBuilder`), `Adapters/` (`OutboundPayloads` builds outgoing `[String: Any]` dictionaries, `ContextPayloadMapper` parses incoming ones — the only two files that know wire key names), `Domain/` (plain `Codable` models).
- Phone side: `modules/watch-connectivity/` is the Expo native module bridge (`index.ts` the JS surface, `ios/WatchConnectivityModule.swift` the native implementation; resolves to `null` on Android, guard every call site). `src/hooks/useWatchCheckInBridge.ts` (weight/water/nutrition context) and `src/hooks/useWatchWorkoutBridge.ts` (workout set completions + heart rate) are the only two consumers; both are mounted headlessly in `App.tsx` (`WatchCheckInGate`/`WatchWorkoutGate`), gated on an active server connection.
- Two transports, chosen per message by durability need: `updateApplicationContext` (`WatchConnectivityModule.updateContext`) is a single latest-value slot for the WHOLE app — nutrition/water/weight context only, never reused for anything else, or a second push would silently clobber it. Everything else (check-ins, water taps, `startWorkout`, `setCompleted`, `heartRateBatch`, `workoutStop`) goes through `sendMessage`-when-reachable-falling-back-to-`transferUserInfo` (`WatchSessionManager.transfer(_:)` on the watch, mirrored ad hoc on the phone for `startWorkout`) — queued, delivered once, safe to layer many independent message types on.
- Renaming a message `type` string requires updating both `OutboundPayloads`/`ContextPayloadMapper` (whichever side sends it) and the matching `route()` switch on the other side (`WatchSessionManager.route` on the watch, `WatchConnectivityModule`'s `WatchSessionDelegateHandler.route` on the phone) — nothing else enforces the pairing.
- Workout tracking: starting a live preset session (`useStartLiveWorkout.ts`) arms the watch with `WatchConnectivity.startWorkout(plan)` — the full exercise/set/target/rest plan, built from the just-started `activeWorkoutStore` state (`buildWatchWorkoutStartPayload`). The watch's Workout tab (`WorkoutView.swift`) is a Hevy-style set tracker (no exercise picker of its own) that starts a real `HKWorkoutSession` for background HR sampling, reports each completed set (`setCompleted`, queued) straight back into `useActiveWorkoutStore.getState().completeSet(setId)` so rest timers/PR detection/autosave dirtying are untouched, and buffers heart-rate batches per exercise (`heartRateBatch`, tagged by whichever exercise is on screen, QUEUED not reachability-gated — a phone out of range mid-workout is normal, and dropping batches would lose the data the feature exists for). Each batch also carries `activeEnergyKcal`, the active energy measured **since the previous batch** — a delta rather than a running total, so the phone sums per exercise and the parts add up to the workout's real total. A workout ends from either side and both flush the same buffer: the watch's Finish Workout (bottom of the exercise-picker sheet) sends `workoutStop`, while finishing/discarding on the phone is caught by `useWatchWorkoutBridge`'s `activeWorkoutStore` subscription, which sends `stopWorkout(sessionId)` the other way so the watch tears its `HKWorkoutSession` down too. Whichever fires first, `flushHeartRate` attaches the buffered series and summed energy per exercise entry via `POST /api/exercise-entries/:id/watch-telemetry` (`attachExerciseEntryWatchTelemetry` in `exerciseApi.ts`) — a route that only fills in avg/max HR, HR zones and `calories_burned` on an entry that already exists, distinct from the HealthKit/Health Connect/Garmin sync path that creates a new entry as part of importing a whole external session. The measured `activeEnergyKcal` REPLACES the server's duration-and-sets calorie estimate for that entry and is also stored in `active_calories`, which is what makes it survive a later edit of the workout (see the server guide's `resolveEditedCaloriesBurned`); heart rate and energy are each optional on that route (HealthKit permissions are per type) but at least one must be present.
- Displaying it: `WorkoutDetailScreen`'s summary row shows **Avg HR** and **Max HR** when any exercise carries one, and `ActiveWorkoutExerciseCard` shows per-exercise heart rate in its chip row next to calories (`142 (168) bpm` — the max in parentheses only when it differs from the average). Both are view-mode only: heart rate has no input anywhere, it only ever arrives from a paired watch or a synced workout. Max HR is the exact maximum of the per-exercise maxima; **Avg HR is an unweighted mean of the per-exercise averages**, because the phone holds only what the server derived per entry and never the raw series, so a duration-weighted figure cannot be computed client-side. Zone breakdowns are still unrendered on mobile — the only zone UI in the repo is the web's `ActivityHeartRateZoneChart` on the Reports page.
- The watch also saves the session to Apple Health: `HKLiveWorkoutBuilder.finishWorkout()` writes a real `HKWorkout`, so a Sparky live workout shows up in Fitness and closes rings. That write has to be excluded from the INBOUND sync or the same session is filed twice — the sets are already in the diary. The exclusion is a metadata marker, not the bundle id: `isOwnRecord` compares against the phone's bundle and the watch app's is `<phone-bundle>.watchkitapp`, so a wrist-written workout looks third-party to it. `WorkoutHealthKitController.sessionMetadataKey` stamps `SparkyFitnessSessionId` onto the workout at `beginCollection`; `healthkit/index.ts` forwards that key into the record's `metadata` (the workout reader carries only the keys a transformer consumes, so it is forwarded explicitly like `HKTimeZone`), and `isOwnWatchWorkout` in `healthkit/dataTransformation.ts` drops it. The key is one literal in Swift and one in TypeScript with no way to share them — renaming either alone silently reintroduces duplicate workouts.

## Testing The Watch In A Simulator

- A watchOS simulator has no heart-rate sensor: `HKWorkoutSession` and `HKLiveWorkoutBuilder` both run, but nothing ever feeds the builder, so `didCollectDataOf` never fires for `.heartRate` and nothing downstream of a reading can be observed. Testing that on hardware needs a paid Apple Developer account, which is why `targets/watch/Infrastructure/SyntheticWorkoutSignal.swift` exists.
- `SPARKY_FAKE_HR=1` in the launch environment makes `WorkoutHealthKitController` substitute that generator for HealthKit, feeding the SAME `pendingSamples` buffer, batch timer and callbacks the real readings use — so what runs is the shipping batching/timestamping/flush path with a different source at the top, not a parallel implementation that could agree with itself while the real one is broken. It also short-circuits `requestAuthorization` to granted, since nothing touches the health store; that keeps `WatchSessionManager.handle(workoutStart:)` on the branch it takes in production. `#if DEBUG` throughout.
- Distinct from `ScreenshotSeed`, which parks one fixed BPM on the store for a still photograph and never reaches this controller. Separate flags that do not interact — a ticking BPM would also defeat the screenshot job's settle check.
- `pnpm run watch:e2e` (`scripts/run-watch.mjs --pair --fake-hr`) is the full rig: it pairs the watch simulator to an iPhone simulator so `WCSession` has a counterpart, then launches the watch app with the synthetic signal. Run order matters — pair first, then `pnpm run ios`, because the phone app evaluates its session against whatever pairing exists. Either flag works alone; bare `pnpm run watch` is unchanged.
- What that covers end to end against a local server: arming from a phone preset, the plan crossing the wire, set completions coming back, heart-rate batches and their per-batch energy delta, the flush on either side's finish, and the `watch-telemetry` POST landing avg/max HR and measured calories on the diary entry.
- `.github/workflows/ios-sim-remote.yml` runs that same rig on a GitHub macOS runner and opens Screen Sharing to it over Tailscale, for when no Mac is to hand. It is the only way to _tap_ the watch app anywhere but a real Mac: Apple does not support UI testing on watchOS, Maestro is iOS-only, and idb does not target watch simulators, so nothing can script a watch simulator's UI. `workflow_dispatch` only, because it holds a runner for the length of the session; it needs `TS_AUTHKEY` and `VNC_PASSWORD` repository secrets, and the phone app is built Debug with Metro running so `__DEV__` is true and a plain-HTTP server URL is accepted.
- What it still does not cover: real sensor cadence and dropouts, `HKWorkoutSession` background behaviour, and the Apple Health round trip — the watch and phone simulators keep separate HealthKit stores, so a workout `finishWorkout()` writes on the watch never reaches the phone's store and the `SparkyFitnessSessionId` duplicate-import guard cannot be exercised there. CI cannot authorize HealthKit at all (`simctl privacy` has no HealthKit service), which is a second reason the synthetic path bypasses it.

## CI Screenshots

- The watch target photographs itself through `targets/watch/Application/ScreenshotSeed.swift`, driven by `SIMCTL_CHILD_*` launch env (`.github/workflows/ios-build.yml`). The phone equivalent is `src/services/screenshotSeed.ts`, and the mechanism differs on purpose: React Native cannot read `simctl` launch env, so the flag is `EXPO_PUBLIC_SCREENSHOT_SEED`, which Expo inlines into the JS bundle at build time.
- When set, `useAppBootstrap` short-circuits to the `WorkoutDetail` route (skipping the server-config lookup that would send an account-less runner to Onboarding) and `App.tsx` supplies `SCREENSHOT_SESSION` as that screen's `initialParams`. Normal navigation always passes its own session, which overrides the initial params, so the flag being compiled out is the only thing that matters in a shipped build.
- The screenshot build is its own parallel job in `ios-build.yml`, not a step in the compile-check job, and is the workflow's only **Release** build. Parallel because the watch scheme builds the containing iOS app with it, so the compile job gets its Debug Pods for free and the Release build then has to build every Pod again — as one job the two overran the timeout, and as two the wall clock is the slower of them rather than their sum. Each uploads its own artifact (`watch-screenshots`, `phone-screenshots`). That is what makes it work at all: `expo-dev-client` owns the first screen of a Debug build, so a Debug run photographs its "Searching for development servers…" launcher however the JS was bundled. The takeover is `#if DEBUG`, so Release goes straight to the app, and the Release build phase embeds the bundle on its own — the app runs in a simulator with **no Metro server**. `FORCE_BUNDLING=1` on a Debug build gets the bundle embedded but never past the launcher, which is the trap to avoid re-entering.
- This only works for screens that take their data from route params and degrade without a server — `WorkoutDetailScreen` falls back to `'kg'` when preferences are unavailable. Diary, Dashboard and the active-workout HUD all need real data and cannot be photographed this way.
- The seed is NOT gated on `__DEV__`, and must not be: the screenshot build is Release, where `__DEV__` is false, so that gate would compile the seed out and put the launcher's replacement — Onboarding — in the shot instead. The env var alone decides, and it is absent from every build but this one. `__tests__/hooks/useAppBootstrap.screenshotSeed.test.ts` covers the branch in its own file, because the flag is a module constant read at import time and cannot be toggled per-test.
