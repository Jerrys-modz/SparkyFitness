...
- The screenshot build is separate from the compile-check build and passes `FORCE_BUNDLING=1`, which embeds the JS so the app runs in a simulator with **no Metro server**. Without it a Debug build shows the red "could not connect" screen and the screenshot is worthless.
...
- The seed is NOT gated on `__DEV__`: the env var alone decides, since whether a force-bundled Debug build counts as a dev bundle is a build detail. `__tests__/hooks/useAppBootstrap.screenshotSeed.test.ts` covers the branch in its own file, because the flag is a module constant read at import time and cannot be toggled per-test.
