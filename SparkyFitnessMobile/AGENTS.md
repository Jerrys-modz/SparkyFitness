...
- Workout tracking: starting a live preset session (`useStartLiveWorkout.ts`) arms the watch with `WatchConnectivity.startWorkout(plan)` — the full exercise/set/target/rest plan, built from the just-started `activeWorkoutStore` state (`buildWatchWorkoutStartPayload`). The watch's Workout tab (`WorkoutView.swift`) is a Hevy-style set tracker (no exercise picker of its own) that starts a real `HKWorkoutSession` for background HR sampling, reports each completed set (`setCompleted`, queued) straight back into `useActiveWorkoutStore.getState().completeSet(setId)` so rest timers/PR detection/autosave dirtying are untouched, and buffers heart-rate batches per exercise (`heartRateBatch`, QUEUED not reachability-gated — a phone out of range mid-workout is normal, and dropping batches would lose the data the feature exists for). Samples come from a long-running `HKAnchoredObjectQuery` (`WorkoutHealthKitController.startHeartRateSeriesQuery`) — NOT a bare `HKQuantitySeriesSampleQuery`, which only enumerates samples that already exist and then completes, so started at workout begin it silently collected nothing on real hardware while the live BPM (fed separately by the builder's statistics) still looked fine; a multi-reading sample is expanded with a series query scoped to that one sample. The query has NO `HKSource.default()` filter — samples recorded during the session are attributed to the watch, not the app, so that filter matched nothing on hardware — and each builder-statistics burst (`didCollectDataOf`) is also appended to the buffer as a floor, so heart rate still reaches the diary if the sample query returns nothing (e.g. HR read access withheld); `appendSample` dedupes by instant. That dedupe set does not survive a relaunch, so the jetsam snapshot records `heartRateSentThrough` and `recoverIfNeeded` resumes the query from it (overlapping, so a multi-reading sample that straddles the mark is still read) and drops readings up to it, instead of replaying the workout from its start. A batch goes out every 60s AND at every exercise boundary (`WorkoutSessionStore.onExerciseWillChange`, fired by the store's single `moveCursor`), so each batch is tagged with the exercise its readings were measured during rather than whichever is on screen when the timer fires. Each batch also carries `activeEnergyKcal`, the active energy measured **since the previous batch** — a delta rather than a running total, so the phone sums per exercise and the parts add up to the workout's real total. A workout ends from either side and both flush the same buffer: the watch's Finish Workout (bottom of the exercise-picker sheet) sends `workoutStop`, while finishing/discarding on the phone is caught by `useWatchWorkoutBridge`'s `activeWorkoutStore` subscription, which sends `stopWorkout(sessionId)` the other way so the watch tears its `HKWorkoutSession` down too. On the phone, buffers are kept PER SESSION (`useWatchWorkoutBridge`'s `sessionsRef`, retained ~10 min after a session ends, max 3): the watch's final drain is queued and often lands after the phone has ended the workout, sometimes after the next one has started, and a single-session buffer dropped it. A 4xx from the attach route drops that entry (it will never succeed) except 401/408/429, which like network/5xx errors keep it for retry; dropped batches and rejected entries are logged. A finish on the WATCH saves and clears the phone's live session (if that save fails the session is left live, so the phone's own Finish can retry it) and calls the bridge's `onWatchFinishedWorkout`, which `App.tsx`'s `WatchWorkoutGate` uses to replace `ActiveWorkout` with `WorkoutComplete` (params from the shared `buildWorkoutCelebration`, `utils/workoutCelebration.ts`); `ActiveWorkoutScreen` also leaves itself whenever its session is cleared from outside while focused, so no external clear can strand it on "No active workout". Whichever fires first, `flushHeartRate` attaches the buffered series and summed energy per exercise entry via `POST /api/exercise-entries/:id/watch-telemetry` (`attachExerciseEntryWatchTelemetry` in `exerciseApi.ts`) — a route that only fills in avg/max HR, HR zones and `calories_burned` on an entry that already exists, distinct from the HealthKit/Health Connect/Garmin sync path that creates a new entry as part of importing a whole external session. The measured `activeEnergyKcal` REPLACES the server's duration-and-sets calorie estimate for that entry and is also stored in `active_calories`, which is what makes it survive a later edit of the workout (see the server guide's `resolveEditedCaloriesBurned`); heart rate and energy are each optional on that route (HealthKit permissions are per type) but at least one must be present.
- Displaying it: `WorkoutCompleteScreen` (the first thing shown when a workout ends) and `WorkoutDetailScreen`'s summary row both show **Avg HR** and **Max HR** when any exercise carries one, via `summarizeWorkoutHeartRate`. `ActiveWorkoutExerciseCard` shows per-exercise heart rate in its chip row next to calories (`142 (168) bpm` — the max in parentheses only when it differs from the average). All three are view-mode only: heart rate has no input anywhere, it only ever arrives from a paired watch or a synced workout. Max HR is the exact maximum of the per-exercise maxima; a missing max is omitted, not shown as a dash. **Avg HR is duration-weighted by each exercise's `duration_minutes`**, so a long block is not pulled down by a short warmup; if any contributing exercise has no positive duration it falls back to a plain mean, because a zero-weight entry would otherwise vanish from it. The phone never holds the raw sample series — the server already duration-weights those into the per-entry average. Zone breakdowns are still unrendered on mobile — the only zone UI in the repo is the web's `ActivityHeartRateZoneChart` on the Reports page.
