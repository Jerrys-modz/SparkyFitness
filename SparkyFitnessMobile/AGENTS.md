...
- `react-native-health-connect` is declared as `^3.5.3`; the installed 3.5.3 build is patched from the repo root via `patchedDependencies` in `pnpm-workspace.yaml`.
...
- Both patched packages are held below their latest major on purpose: `react-native-health-connect` has a 4.x and `@bacons/apple-targets` a 5.x, and moving to either invalidates its patch file. `expo install --fix` does not touch them (neither is an Expo SDK package), so the SDK 57 upgrade left both patches applying cleanly.
