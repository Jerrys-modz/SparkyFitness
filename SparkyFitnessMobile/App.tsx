...
/**
 * Unlike `WatchCheckInGate`, listeners stay up while the server is offline so
 * a set or heart-rate batch is not dropped. The /health poll only runs when
 * WatchConnectivity is actually available and a telemetry buffer still needs
 * a flush — Android and an idle iPhone do not poll for a feature they are
 * not using.
 */
        ...
        <LightboxProvider>
          ...
          >
            ...
            <Stack.Screen
              ...
              options={createStackScreenOptions(
                t('waterContainerEdit.editTitle', {
                  defaultValue: 'Edit container',
                }),
                { headerBackTitle: t('common.back', { defaultValue: 'Back' }) }
              )}
            />
            ...
            <Stack.Screen
              ...
              options={createStackScreenOptions(
                t('foodEntryMultiAdd.title', { defaultValue: 'Review' }),
                {
                  headerBackButtonDisplayMode: 'minimal',
                }
              )}
